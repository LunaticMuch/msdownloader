...
def get_ms_funds_prices(SAL, JWT, funds_list):
    ...
    universe_list = '|'.join(
        [str(universe) for universe in pydash.arrays.sorted_uniq(output_dict)])
...
def get_currencies(currencies_list):
    output_dict = []
    for d in currencies_list:
        item_dict = {}
        ...
        item_dict['ClosePriceDate'] = prices['date']
        item_dict['ClosePrice'] = round(float(prices['rates'][code_cur]), 4)
        item_dict['PriceCurrency'] = base_cur
...
def print_prices(prices):
    for security_price in prices:
        close_price = str(round(float(security_price['ClosePrice'])/100, 4)
                          if my_args.x is True and security_price['PriceCurrency'] == 'GBX' else security_price['ClosePrice'])
        price_currency = str(
            'GBP' if my_args.x is True and security_price['PriceCurrency'] == 'GBX' else security_price['PriceCurrency'])
...
def read_args():
    arg_parser = argparse.ArgumentParser(
        description='Download securities latest price from Morningstar')
    group_input = arg_parser.add_mutually_exclusive_group(required=True)
    group_input.add_argument('-c', action='store', metavar='securities.yaml',
                             default='securities.yaml', type=str, help='The YAML file with the list of securities')
    group_input.add_argument('-d', action='store', metavar='XY03ID03131ID',
                             help='Dump info for a single security in JSON')
    group_input.add_argument('-l', action='store', metavar='GB00B0XWNK36',
                             help='Lookup for a security and print the simple data')
    arg_parser.add_argument('-x', action='store_true',
                            help='Force conversion from pence sterling GBX into pound sterling GBP')
    arg_parser.add_argument('-b', action='store_false',
                            help='Return beancount format instead of ledger/hledger')
    arg_parser.add_argument('-o', action='store', metavar='output.txt',
                            type=str, help='The output file for the latest prices')
    arg_parser.add_argument('-w', action='store_true',
                            help='Trucate the output file if exists and add new prices')
...
if my_args.d is not None:
    print(json.dumps(get_ms_security_snapshot(
        auth[0], auth[1], my_args.d), indent=4, sort_keys=True))
elif my_args.l is not None:
    print(json.dumps(get_ms_security_data(
        auth[0], auth[1], my_args.l), indent=4, sort_keys=True))
else:
    securities = read_securities(my_args.c)
    prices = []
    for group in securities.keys():
        if group == 'funds':
            funds_prices = get_ms_funds_prices(
                auth[0], auth[1], securities['funds'])
            prices = funds_prices
        elif group == 'shares':
            shares_prices = get_ms_shares_prices(
                auth[0], auth[1], securities['shares'])
            prices.extend(shares_prices)
        elif group == 'currencies':
            currencies_prices = get_currencies(securities['currencies'])
            prices.extend(currencies_prices)
        else:
